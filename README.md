...
* 클린코드 8장을 읽다가 Adapter 패턴을 사용하여 경계에 위치하는 코드를 분리하라는 저자의 말이 있었다.
* Adapter 패턴이 생각나지 않아서 따로 정리하고자 한다.

* 어뎁터 패턴 : **호환되지 않는 인터페이스들을 연결하는 디자인 패턴**

## 팩토리 패턴

* 클린코드 11장을 읽다가 객체 생성을 클래스로 캡슐화 처리하여 생성하게 할 때 Factory 패턴을 사용하여 한다는 문구를 읽고나서 Factory 패턴을 자세하게 알아보기 위해 정리하고자 합니다.
